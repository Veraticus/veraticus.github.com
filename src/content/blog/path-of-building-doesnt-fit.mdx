...
Try it yourself. Drag a block into the Worker.
...
So. Some background, briefly, for the non-PoE-brained among you. Path of Building (PoB) is a community-maintained desktop application that simulates your character against every damage formula in a game that has, conservatively, a thousand of them. You paste in a build, it spits out your DPS, your survival numbers, your resistances, and the exact moment you stop being able to afford the next gem upgrade. It is the calc engine the entire community runs on. If you have ever read a build guide that quoted a number, the number came from PoB. It is a rare and wonderful piece of software -- open source, maintained by people who genuinely love the game, never taken hostage by a storefront, and weighing in at about two hundred thousand lines of Lua that somehow still works.

I wanted Claude to be able to use it.

Here is what I mean by "use it." A build guide says `take Vaal Pact at level 48`. Fine. But *your* build -- the one you are actually playing, with the gear you actually own -- might not want Vaal Pact. Maybe it wants Doctrine of the Unyielding instead. Maybe you don't have the stat investment for either one. Maybe it's just a bad build, which is why you're reading the guide in the first place. A smart, patient friend could load your build into PoB, try a few permutations, and eventually -- if they liked you enough -- tell you which one helped. That is what I wanted the AI to do. I wanted the LLM to hold a pickaxe at the calc engine until the calc engine gave up a DPS number, then swap a passive and go again, and again, and again, until it had something useful to tell me.

The naive version of this is obvious: embed PoB in my Worker, call into it from the MCP tool handler, stream the result back. I host my whole stack on Cloudflare's edge. Workers are cheap and globally distributed and fast enough for what I'm doing. I had already embedded two other games' reference engines as WASM modules. This would make three. Get the Lua compiled to WASM, stuff it into a Worker, done.
