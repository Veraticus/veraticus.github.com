...
The five blocks in the widget above are the weight distribution of that Lua, roughly. **Mod tables**: every prefix and suffix PoE can roll, pre-parsed. **Uniques**: every unique item in the game, parsed through the mod engine so the calc can read them directly. **UI + class defs**: the rendering layer plus the seven base classes and their ascendancies. **Calc temps**: scratch memory the damage math holds while a single evaluation runs. **Your build**: the character, gear, gems, and passive tree you pasted in.
